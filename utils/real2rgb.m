function [B] = real2rgb(A, cmap, lims)
...
% Source: MATLAB Central File Exchange (Oliver Woodford).
% License: BSD-2-Clause (see THIRD_PARTY_LICENSES.md).
...
%[B lims] = rescale_sat(B, lims, [0 maxInd]);
B = rescale_sat(B, [min(B(:)) max(B(:))],[0, maxInd]);
